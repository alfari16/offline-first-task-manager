...
export default class PouchyStore {
  ...
  watchRemote() {
    ...
    this.handlerRemoteChange = this.dbLocal.replicate
      .from(this.dbRemote, {
        live: true,
        retry: true,
      })
  ...
  watchLocal() {
    this.handlerLocalChange = this.dbLocal
      .changes({
        since: 'now',
        live: true,
        include_docs: true,
  ...
  watchMeta() {
    this.handlerMetaChange = this.dbMeta
      .changes({
        since: 'now',
        live: true,
        include_docs: true,
  ...
  async deleteItem(id, user = {}) {
    ...
    const isRealDelete =
      doc.deletedAt || doc.createdAt > this.dataMeta.tsUpload;
    if (isRealDelete) {
  ...
  async editSingle(payload) {
    const doc = (await this.dbLocal.getFailSafe(this.single)) || {
      _id: this.single,
    };
    await this.dbLocal.put({
      ...doc,
